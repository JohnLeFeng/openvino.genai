...
######################################################
# Simple standalone wrapper used by benchmark.py


class MemMonitorWrapper:
    """Lightweight wrapper around two MemoryMonitor instances (RSS + System).

    Provides the minimal interface expected by benchmark.py without requiring
    an argparse Namespace object.

    Usage::

        monitor = MemMonitorWrapper()
        monitor.memory_unit = MemoryUnit.GiB
        monitor.create_monitors()
        monitor.start()
        # ... work ...
        monitor.stop_and_collect_data("stage")
        rss_max, rss_inc, sys_max, sys_inc = monitor.get_data()
        monitor.start()          # restart for next stage
        ...
    """
    ...
    def __init__(self, interval: float = DEFAULT_INTERVAL) -> None:
        self.interval = interval
        self.memory_unit: MemoryUnit = MemoryUnit.GiB
        self._rss_monitor: Optional[MemoryMonitor] = None
        self._sys_monitor: Optional[MemoryMonitor] = None
        self._rss_max: float = -1.0
        self._rss_increase: float = -1.0
        self._sys_max: float = -1.0
        self._sys_increase: float = -1.0

    def create_monitors(self) -> None:
        """(Re-)create MemoryMonitor instances with the current memory_unit."""
        self._rss_monitor = MemoryMonitor(
            interval=self.interval,
            memory_type=MemoryType.RSS,
            memory_unit=self.memory_unit,
        )
        self._sys_monitor = MemoryMonitor(
            interval=self.interval,
    ...
    def start(self) -> None:
        """Start (or restart) memory monitoring threads."""
        if self._rss_monitor is None:
            self.create_monitors()
        self._rss_monitor.start()
        self._sys_monitor.start()

    def stop_and_collect_data(self, stage: str = "") -> None:
        """Stop monitors and record peak / increase values."""
        for monitor, attr_max, attr_inc in (
            (self._rss_monitor, "_rss_max", "_rss_increase"),
            (self._sys_monitor, "_sys_max", "_sys_increase"),
        ):
            if monitor is None:
                continue
            monitor.stop()
            _, values_abs = monitor.get_data(memory_from_zero=False)
            _, values_rel = monitor.get_data(memory_from_zero=True)
            setattr(self, attr_max, max(values_abs) if values_abs else -1.0)
            setattr(self, attr_inc, max(values_rel) if values_rel else -1.0)

    def get_data(self) -> tuple:
        """Return (max_rss, rss_increase, max_sys, sys_increase) in memory_unit."""
        return self._rss_max, self._rss_increase, self._sys_max, self._sys_increase
