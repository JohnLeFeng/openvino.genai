...
def main():
    ...
    parser.add_argument("-i", "--image", type=str, default="test_cat.png", help="Image")
    parser.add_argument("--image_size", type=int, nargs=2, metavar=('width', 'height'), help="Resize image to the specified width and height")
    ...
    pipeline_config = {
        ...
        "DEVICE_PROPERTIES": {
            "NPU": {
                ...
                "NPU_BYPASS_UMD_CACHING" : "YES"
            },
            "AUTO": {"MULTI_DEVICE_PRIORITIES": "GPU,CPU"},
        }
    }
    ...
    images = read_images(args.image, resize=tuple(args.image_size) if args.image_size else None)
    device = args.device
    ...
    print (f"Max RSS Memory consumption: {max_rss_mem_consumption:.2f} {memory_unit}, Max RSS Memory increase: {max_rss_mem_increase:.2f} {memory_unit}")
    print (f"Max System Memory consumption: {max_sys_mem_consumption:.2f} {memory_unit}, Max System Memory increase: {max_sys_mem_increase:.2f} {memory_unit}")
    memory_monitor.start()

    pipe = ov_genai.VLMPipeline(models_path, device, config=pipeline_config)
    ...
    print(f"Image size: {images[0].get_shape()[1]}, {images[0].get_shape()[2]}")
    print(f"Output: {res.texts[0]}")
